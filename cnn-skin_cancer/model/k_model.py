...
def run_model(mlflow_run_id, X_train, y_train, X_test, y_test,**kwargs):
  ...
  params = {'num_classes' : 2, #len(class_names)
  ...
  model = Sequential([
    ...
    layers.Conv2D(64, kernel_size=(3, 3),padding = 'Same',input_shape=params.get('input_shape'), activation=params.get('activation'), kernel_initializer=params.get('kernel_initializer')),
    layers.MaxPooling2D(pool_size = (2, 2)),
    layers.Dropout(0.25),
    layers.Conv2D(64, kernel_size=(3, 3),padding = 'Same', activation =params.get('activation'), kernel_initializer = params.get('kernel_initializer')),
    layers.MaxPooling2D(pool_size = (2, 2)),
    layers.Dropout(0.25),
    layers.Flatten(),
  ...
  with mlflow.start_run(run_id=mlflow_run_id) as run:
    ...
    history = model.fit(X_train, y_train, validation_split= params.get('validation_split'),
                        epochs= params.get('epochs'), batch_size= params.get('batch_size'), verbose=1, 
